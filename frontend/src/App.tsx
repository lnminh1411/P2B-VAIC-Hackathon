...
export default function App() {
  ...
  return (
    <div className="flex flex-col min-h-screen text-slate-100 bg-slate-950">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <select 
                ...
                className="px-3 py-1.5 bg-slate-800 border border-slate-700 rounded-md text-sm text-white focus:outline-none focus:border-indigo-500"
              >
                {companies.map(c => (
      ...
      {/* Main Container */}
      <main className="flex-1 px-6 py-8 mx-auto max-w-7xl w-full grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <section className="lg:col-span-4 flex flex-col gap-6">
          <div className="glass-card p-6 flex flex-col gap-4">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              ...
              <span className="text-[10px] text-indigo-400 font-mono tracking-wider">VERIFIABLE FACTS</span>
            </div>
          ...
          {companyPassport && selectedField && (
            <div className="glass-card p-6 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-3 text-sm text-left">
                <div>
                  <span className="text-xs text-slate-500">Evidence Quote:</span>
                  <blockquote className="mt-1 pl-3 border-l-2 border-indigo-500 italic text-slate-300 text-xs py-1 bg-indigo-950/20 rounded-r">
                ...
                <div className="text-[11px] text-slate-500 mt-2 flex items-center justify-between">
                  <span>URI: {companyPassport[selectedField].source_uri || 'N/A'}</span>
                  <span>Observed: {new Date(companyPassport[selectedField].observed_at).toLocaleDateString()}</span>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                {eligibility ? (
                  <div className="flex flex-col gap-3">
                          ...
                          >
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                ...
                                <span className="block text-[10px] text-slate-500 font-mono mt-0.5">ID: {rule.rule_id}</span>
                              </div>
                            </div>
                          </div>
                        );
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-800">
                      <h4 className="text-xs font-bold text-slate-400 mb-2 uppercase tracking-wide">Danh mục hồ sơ yêu cầu:</h4>
              ...
              <div className="glass-card p-6 flex flex-col gap-4 text-left">
                ...
                {activeRuleDetail ? (
                  <div className="flex flex-col gap-4 text-xs">
                    ...
                    <div className="p-3 bg-slate-900/60 rounded border border-slate-800">
                      <div className="flex justify-between items-center mb-2">
                        <span className="font-semibold text-indigo-400 uppercase tracking-wide text-[10px]">Company Evidence</span>
                        <span className="text-slate-500">Field: {activeRuleDetail.field}</span>
                      </div>
                      <div className="text-sm font-bold text-white mb-2">
                        Giá trị thực tế: {activeRuleDetail.actual_value !== undefined ? activeRuleDetail.actual_value : 'N/A'}
                      </div>
                      <blockquote className="pl-3 border-l-2 border-indigo-500 italic text-slate-300 text-xs">
                        "{activeRuleDetail.evidence_quote || 'Không có bằng chứng được trích dẫn'}"
                      </blockquote>
                      <div className="text-[10px] text-slate-500 mt-2">
                        Nguồn: {activeRuleDetail.source_uri} ({activeRuleDetail.source_location})
                      </div>
                    </div>
                    ...
                    <div className="p-3 bg-slate-900/60 rounded border border-slate-800">
                      <div className="flex justify-between items-center mb-2">
                        <span className="font-semibold text-emerald-400 uppercase tracking-wide text-[10px]">Policy Clause</span>
                        <span className="text-slate-500">Operator: {activeRuleDetail.operator}</span>
                      </div>
                      <div className="text-sm font-bold text-white mb-2">
                        Điều kiện: {activeRuleDetail.operator} {activeRuleDetail.expected_value}
                      </div>
                      {activeRuleDetail.citation ? (
                        <>
                          <blockquote className="pl-3 border-l-2 border-emerald-500 italic text-slate-300 text-xs">
                            "{activeRuleDetail.citation.quote}"
                          </blockquote>
                          <div className="text-[10px] text-emerald-400/80 mt-2 flex justify-between items-center">
                            ...
                            >
                              Văn bản gốc <ExternalLink className="w-2.5 h-2.5" />
                            </a>
                          </div>
                        </>
                      ) : (
                        <p className="text-slate-500 italic">Không có trích dẫn văn bản</p>
                      )}
                    </div>
                    ...
                    <div className={`p-3 rounded border flex items-center gap-3 ${
                      ...
                      <div>{getStatusIcon(activeRuleDetail.status)}</div>
                      <div>
                        <span className="font-bold">Đánh giá tiêu chí: {activeRuleDetail.status}</span>
                        <p className="text-[11px] mt-0.5 text-slate-300">{activeRuleDetail.reason}</p>
                      </div>
                    </div>

                    {/* Clarification Loop for MISSING_INFO */}
                    {activeRuleDetail.status === 'MISSING_INFO' && (
                      <div className="p-3 bg-amber-500/5 border border-amber-500/20 rounded flex flex-col gap-3">
                        <div className="flex items-center gap-2 text-amber-400 font-semibold">
                          <HelpCircle className="w-4 h-4" />
                          <span>Hỏi làm rõ thông tin:</span>
                        </div>
                        <p className="text-slate-300">{activeRuleDetail.clarification_question}</p>
                        <div className="flex items-center gap-2">
                          <input 
                            type="text"
                            ...
                            className="flex-1 px-3 py-1.5 bg-slate-900 border border-slate-800 rounded text-xs text-white focus:outline-none focus:border-indigo-500"
                          />
                          <button 
                            onClick={() => submitClarification(activeRuleDetail.field)}
                  ...
                  <div className="py-8 text-center text-slate-500 text-sm">Select a criterion to view evidence</div>
                )}
              </div>
            </div>
          )}
                    ...
                    <div className="flex flex-col gap-3">
                      <div>
                        ...
                        <textarea 
                          ...
                          className="w-full px-3 py-2 bg-slate-900 border border-slate-800 rounded text-xs text-white focus:outline-none focus:border-indigo-500 h-16"
                        />
                      </div>
                      ...
                      <div className="flex justify-end gap-3">
                        ...
                        <button 
                          ...
                          className="flex items-center gap-2 px-4 py-1.5 text-xs bg-indigo-600 hover:bg-indigo-500 text-white rounded transition-colors glowing-btn"
                        >
      ...
      {editingField && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="glass-card p-6 w-full max-w-md flex flex-col gap-4 text-left">
            ...
            <div className="flex flex-col gap-2">
              ...
              <input 
                ...
                className="w-full px-3 py-2 bg-slate-900 border border-slate-800 rounded text-sm text-white focus:outline-none focus:border-indigo-500"
              />
            ...
            <div className="flex justify-end gap-3 mt-4">
              ...
              <button 
                ...
                className="px-4 py-1.5 text-xs bg-indigo-600 hover:bg-indigo-500 text-white rounded transition-colors glowing-btn"
              >
                Lưu Thay Đổi
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="glass-card p-6 w-full max-w-2xl flex flex-col gap-4 text-left">
            ...
            <div className="overflow-y-auto max-h-96 flex flex-col gap-2.5">
              {auditLogs.length > 0 ? (
                auditLogs.map((log) => (
                  <div key={log.id} className="p-3 bg-slate-900 border border-slate-800 rounded text-xs flex flex-col gap-1.5">
                    <div className="flex justify-between items-center">
