...
import { Check, ChevronRight, FileSearch, PencilLine, Quote, RefreshCw, ShieldCheck } from 'lucide-react'
...
export function PassportPage({ passport, candidates, onConfirm, busy }: { passport: Passport; candidates: Candidate[]; onConfirm: (candidate: Candidate) => Promise<void>; busy: boolean }) {
  ...
  return (
    <>
      <section className="page-heading split-heading"><div><span className="kicker">COMPANY PASSPORT · VERSION {passport.version}</span><h1>Hồ sơ doanh nghiệp <em>có thể kiểm chứng.</em></h1><p>Mỗi dữ kiện đi cùng bằng chứng, mức tin cậy và lịch sử thay đổi.</p></div><div className="heading-actions"><button className="button secondary"><RefreshCw />Cập nhật nguồn</button>{pending.length > 0 && <button className="button primary" disabled={busy} onClick={confirmAll}><Check />Xác nhận tất cả ({pending.length})</button>}</div></section>
      <div className="passport-summary"><div className="company-monogram">GT</div><div><h2>{passport.company_name}</h2><p>{passport.website}</p><span>Workspace riêng tư · cập nhật {formatDate(passport.updated_at)}</span></div><div className="passport-score"><strong>{Object.values(passport.fields).filter(field => field.status === 'CONFIRMED').length}/{Object.values(passport.fields).length}</strong><span>dữ kiện xác nhận</span></div></div>
      <Tabs.Root className="passport-tabs" defaultValue="fields">
        ...
        <Tabs.Content value="history"><div className="timeline"><div><b>v{passport.version}</b><span><strong>Phiên bản hiện tại</strong>Đã cập nhật từ xác nhận của người dùng</span></div><div><b>v1</b><span><strong>Khởi tạo Passport</strong>Tạo từ tên doanh nghiệp, website và tài liệu PDF</span></div></div></Tabs.Content>
      </Tabs.Root>
    </>
  )
}
