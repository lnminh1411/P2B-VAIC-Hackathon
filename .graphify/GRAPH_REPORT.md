...
## Corpus Check
- Corpus is ~27,024 words - fits in a single context window. You may not need a graph.

## Summary
- 169 nodes · 299 edges · 11 communities detected
- Extraction: 64% EXTRACTED · 36% INFERRED · 0% AMBIGUOUS · INFERRED: 109 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output
- Edge kinds: uses: 109 · contains: 80 · rationale_for: 42 · inherits: 25 · calls: 20 · method: 20 · imports_from: 3


## Input Scope
- Requested: auto
- Resolved: committed (source: default-auto)
- Included files: 41 · Candidates: 125
- Excluded: 80 untracked · 5172 ignored · 0 sensitive · 0 missing committed
- Recommendation: Use --scope all or graphify.yaml inputs.corpus for a knowledge-base folder.

## Graph Freshness
- Built from Git commit: `9c0b749`
...
Cohesion: 0.23
Nodes (21): Evaluates a single deterministic rule against the CompanyPassport.     Returns, Recursively evaluates a RuleGroup using ALL/ANY logic.     Returns (status, gro, ExtractedPolicyOpportunity, fetch_external_decrees(), get_fallback_mock_decree(), construct_policy_rules(), search_and_cache_decrees(), Attempts to search and scrape decrees from vbpl.vn.     Returns a list of dicts (+13 more)

### Community 1 - "Community 1"
Cohesion: 0.06
Nodes (13): run_daily_cache_sync(), daily_cron_worker(), is_allowed_origin(), dynamic_cors_middleware(), extract_document(), extract_multiple_documents(), sync_legal_documents(), Single file upload extraction endpoint (Goal 2) (+5 more)
...
Cohesion: 0.50
Nodes (1): translations

## Knowledge Gaps
- **19 isolated node(s):** `Single file upload extraction endpoint (Goal 2)`, `Intelligent multi-document sorting and extraction agent (Goal 7)`, `Verifiable SHA-256 diff hash policy sync engine (Goal 3)`, `Finds the most recent year (e.g. 2024, 2025) mentioned in the document text.`, `Ranks documents based on semantic relevance to the query and date recency,     r` (+14 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 10`** (2 nodes): `get_db_connection()`, `init_db()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 11`** (2 nodes): `convert_to_markdown_local()`, `Converts a document (docx, xlsx, doc, csv, pptx, html, etc.) to markdown using l`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 6`** (1 nodes): `TestActiveSearchAndCache`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 5`** (1 nodes): `TestP2BGoldenPath`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 8`** (1 nodes): `translations`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
