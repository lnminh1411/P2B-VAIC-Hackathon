...
CREATE TABLE workspaces (
    ...
    owner_subject uuid NOT NULL UNIQUE REFERENCES auth.users(id) ON DELETE CASCADE,
...
CREATE TABLE policy_versions (
    ...
    reviewer_subject uuid REFERENCES auth.users(id),
    created_at timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE audit_events (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    workspace_id uuid REFERENCES workspaces(id) ON DELETE CASCADE,
    actor_subject uuid NOT NULL REFERENCES auth.users(id),
...
-- Business data is never accessed from browser Data API. Railway owns all
-- authorization decisions after verifying the Supabase bearer token.
REVOKE ALL ON ALL TABLES IN SCHEMA public FROM anon, authenticated;
REVOKE ALL ON ALL SEQUENCES IN SCHEMA public FROM anon, authenticated;

INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
VALUES ('p2b-private', 'p2b-private', false, 20971520, ARRAY['application/pdf', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'])
