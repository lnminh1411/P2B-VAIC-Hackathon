...
- Railway: build bằng [`infra/Dockerfile`](infra/Dockerfile) và [`railway.json`](railway.json); pre-deploy tự chạy migration. Entrypoint whitelist process theo `P2B_PROCESS`: API dùng mặc định `api`, extraction worker đặt `worker`; API readiness endpoint là `/health/ready`.
